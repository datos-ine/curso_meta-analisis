...
$$\theta = \frac{\sum{y_k w_k}}{\sum{w_k}} \qquad donde~w_k = \frac{1}{S^2_{y_k}}$$ {#eq-2}
...
Dado que las y los participantes del curso tienen distintas formaciones y niveles de experiencia en R, durante las clases utilizaremos el paquete `meta`, que proporciona una interfaz sencilla para ajustar modelos de meta-análisis y se basa internamente en las funciones del paquete `metafor`.

Quienes tengan mayor experiencia en programación y deseen contar con más flexibilidad para especificar modelos, realizar análisis avanzados o personalizar gráficos y resultados, podrán trabajar directamente con el paquete `metafor`.
...
Usaremos el conjunto de datos `Curtin2002`, incluido en la dependencia `metabook` [@metabook]. El mismo presenta los resultados de 21 estudios que evalúan el efecto de la suplementación con potasio sobre la presión diastólica.
...
# Comprobar si cada estudio tiene un identificador
tabyl(datos$author)
```

Las variables de entrada para `metagen()` serán `mean` (diferencia de medias estandarizada) y `SE` (variabilidad de la estimación). Como `author` se repite, debemos crear un identificador único del estudio combinando `author` y `year`:

```{r}
#| id: mod-1
# Crear id de estudio
datos <- datos |> 
  mutate(au_anio = paste0(author, " (", year, ")"))
...
Antes de analizar en detalle cada elemento de la salida debemos fijarnos en el valor de $I^2$, ya que si el porcentaje de heterogeneidad estadística es alto, debemos omitir los resultados del modelo de efectos fijos. En nuestro ejemplo $I^2 = 92,5\%$ nos indica que el modelo de efectos aleatorios es el más apropiado (podemos reajustar el modelo cambiando el argumento `common` a `FALSE` o simplemente ignorar estos coeficientes).
...
En base a la salida anterior, podemos concluir que el meta-análisis realizado sobre 21 estudios individuales muestra que la presión diastólica disminuye significativamente cuando se administran suplementos de potasio ($p < 0.005$). La alta heterogeneidad estadística ($I^2 = 92,5\%$) sugiere que la variabilidad observada se debe a diferencias reales entre estudios y el intervalo de predicción incluye la posibilidad de obtener asociaciones negativas en futuros estudios.
...
#| fig-width: 9
forest(mod,
       smlab = "Dif. de medias estandarizada",
...
- Muestra el **identificador único de cada estudio** (generalmente el apellido del primer autor y/o el año de publicación).
...
Para evitar este problema, se recomienda exportar el gráfico a un archivo de imagen (por ejemplo, PDF o PNG) usando los argumentos `filename`, `path`, `device`, `height`, `width`, `units` y `dpi` . En la última clase además veremos opciones de visualización avanzada para generar gráficos listos para su publicación.
