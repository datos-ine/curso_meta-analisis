---
title: "Meta-análisis en estudios analíticos"
---
...
En esta sección repasaremos los principales estimadores de efecto utilizados en estudios epidemiológicos analíticos, junto con ejemplos prácticos para su ajuste e interpretación en R.
...
Como ejemplo, utilizaremos la base de datos `dat.poole`, que contiene resultados de 67 estudios sobre la efectividad de agentes mucolíticos en pacientes con bronquitis crónica o EPOC:
...
- `ne`: tamaño muestral en grupo tratamiento.

- `me`: media en grupo ratamiento.

- `se`: desvío estándar de la media en grupo tratamiento.

- `nc`: tamaño muestral en grupo control.

- `mc`: media muestral en grupo control.

- `sc`: desvío estándar de la media en grupo control.
...
```{r}
#| id: metacont-2
# Crear identificador único de estudio
datos_md <- datos_md |>
...
Los coeficientes del modelo de efectos aleatorios indican que el grupo tratado con mucolíticos presenta una reducción en los síntomas respecto del grupo control $(p<0,001)$. El intervalo de predicción muestra que es posible que estudios futuros encuentren el efecto inverso al estimado.
...
forest(
  ...
  leftlabs = c("Estudio", rep(c("Total", "Media", "SD"), 2)), # <3>
...
forest(
  ...
  leftcols = "studlab", # <4>
  leftlabs = "Estudio", # <5>
  rightlabs = c("IRR", "95% IC", "Peso (fijo)", "Peso (aleatorio)"), # <6>
